...
// no explanation at the top, a planted bug nobody reads, a report page that no
// longer matches the data it renders. Formatting arguments are deliberately
// absent.
//
//   node tools/lint.mjs
...
function walk(dir, acc = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (['node_modules', '.git', '.playwright-mcp'].includes(entry.name)) continue;
      walk(full, acc);
    } else if (/\.(js|mjs)$/.test(entry.name)) {
      if (entry.name === 'data.js') continue; // generated
      acc.push(full);
    }
  }
  return acc;
...
function main() {
  ...
  for (const file of files) {
    ...
    // Library modules return values; only the CLI, the tools and the target
    // harness are allowed to speak.
    const isLibrary = rel.startsWith('src/') && rel !== 'src/cli.js' &&
      rel !== 'src/targets/child.js' && rel !== 'src/targets/run.js';
  ...
  // The report page must not reach the network: it is meant to open from
  // file:// with no server, and a fetch would break that silently.
